...
# fit and save results  
  if(mod %in% c("ht","aht","abur","koh","akoh")){
    ...
    if (conv %in% c(1,2,3)) {   
        ...
        loglik = -log(2*pi)*nn/2 - log(var(yy))*nn/2 - var(yy)*(nn-1)/(2*var(yy))
        aic = 2*npar - 2*loglik
        bic = npar*log(nn) - 2*loglik
    } else {
        rmse = NA
        aic = NA
        bic = NA
...
fits = function(mod,lower_shelf,upper_shelf,yy,temp,fit){
